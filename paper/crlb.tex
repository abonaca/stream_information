...
% show what properties of more flexible potentials are constrained by stellar streams
% These are radically different scenarios 
...
* how it fits with the literature
...
-- intuition that in flexible -- zeroing in on the stream; comes from stream==orbit
...
- in current format:
- most efficient stream / best place along the stream to get extra observations
- lsst era, when too many streams will be known to follow up every one of them
- m31 / extragalactic applications

- influence of LMC/Sgr in the CRLB formalism
-- need time dependent bfe self-consistently, otherwise building in wrongness (like lm10)
-- need to maintain thin and old disk
-- potential complexities need to be realistic, not adding noise to the problem
-- lmc perturbing on timescale these streams created
-- need to test in a live potential, model streams with a toy, bfe, where is the answer better?
-- self-consistent to put in external things
-- can we make the case for flexible + time-evolving models with ophiuchus, or perhaps some of the streams close to the LMC?
-- posit whether galaxy simulations would provide good models, or just a basis in which we evolve individual subhalos

* return to the assumptions paper makes and criticize them
- placeholder in discussion: crb w noisy derivatives -> overly optimistic information (reason why so much time spent on derivatives)
...
-- here: not modeling streams with actual data -- modeling toys comparable to streams we know about
-- haven't done the fitting, asking how precise would be if we did the fitting
-- credibly naive model of what the data are, reality more complicated, places with better measurements along the stream (membership not required?), assuming we can measure the track well in 6D, this will be true/not for these reasons in Gaia era
-- also assuming that the noise is well behaved (noise gaussian) -- could be non-trivial error issues in real data (distances from parallax)
-- gravitational potential is a toy (although people have been using them)
...
- full stream simulating, expensive
-- numerical derivatives, autodif, also good for hmc
...
- naive way forward:
-- lay down information for local acceleration 
-- interpolate between those with a model (role of bfe, make sure these values are physically realizable)
