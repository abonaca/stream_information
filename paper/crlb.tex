...
% so we chart the way forward by discussing constraints streams place on more flexible models of the Galactic gravitational potential.
% The \CRLB\ on any one parameter depends strongly on the model freedom; as we permit more potential freedom, the information about, say, halo triaxiality, reduces.
% We perform experiments to demonstrate this using potential basis functions that permit great freedom in the gravitational potential on intermediate scales.
% We make some comments about the information value of various new observations that could be made of particular known streams.

% We first show that kinematic measurements of stream stars are essential for producing competitive constraints on the distribution of matter. Furthermore, in simple, analytic models of the Milky Way, streams are most sensitive to the total mass enclosed within the progenitor's orbit. Known streams are orbiting the inner 50 kpc of the Galaxy, so even though the precision of gravitational potential recovery can be improved by an order of magnitude when simultaneously fitting multiple streams, this measurement is still mapping less than a percent of the virial volume. This landscape will have changed in the era of extremely large telescopes, as the next generation of photometric surveys, such as LSST, are expected to discover cold streams out to hundreds of kpc. 20-m-class telescopes will be vital in providing the necessary kinematic information for these faint targets. As the outer halo is predicted to be asymmetric, we discuss how the precision in measured radial velocities would affect mapping of dark matter in the Galactic outskirts, both in terms of overall precision, as well as the complexity of the halo model allowed by these data.

% We find that, in simple, static, analytic models of the Milky Way, streams XX and YY contain the most information about the dark-matter shape.
% For any individual stream, there are near-degeneracies between dark-matter halo properties and other parameters, including the mass of the Large Magellanic Cloud, the total dark-matter mass, and other potential parameters, but we find that simultaneous fitting of multiple streams ought to precisely constrain all parameters well.
...
Given the model parameters $\vec{x}$ (e.g., a vector with the mass and scale radius of a dark matter halo),  the covariance matrix for the model parameters, $C_x$, is the Fisher information matrix \citep{fisher}:
\begin{equation}
...
The \CRLB\ for model parameters $\vec{x}$ are the diagonal elements of the Fisher matrix.
Through the rest of this section, we describe individual terms of Equation~\ref{eq:crlb} in the context of stellar streams: first, we present the model of the Galaxy and the existing constraints on its components (\S\ref{sec:model}), then the change in stream observables $\vec{y}$ as a function of changes in model parameters $\vec{x}$, i.e., the derivative $d\vec{y}/d\vec{x}$ (\S\ref{sec:derivatives}), and finally the adopted observational uncertainties, which set the covariance matrix $C_y$ (\S\ref{sec:datasets}).
...
These studies have shown that the phase-space distribution of the resulting debris is predominantly set by properties of the gravitational potential and the orbit of the progenitor, with a weaker dependence on the internal properties of the progenitor (ahwk).
...
% - these questions can now be explored on formal footing
% - streams analyzed so far are available at following url
...
% -- this is now indicated in 1d, but should be tested in 3d with fully flexible models, which we discuss in more detail in \S\,
