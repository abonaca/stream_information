...
% - most flexible -- expansion
% - shown to reproduce Nbody + hydro
% - but there is a conceptual hurdle: we know LMC is perturbing on timescale these streams created
% - and it seems this time evolution treated in a static potential can result in recovered shapes being on average correct, but are difficult to interpret and match to properties of the Milky Way's own dark matter halo
% - a robust way to tackle this is using time-dependent bfe, that still maintains a thin and old disk (nico)
...
% - nico's decomposed MW and LMC interaction , finds that different areas differently affected -- need to have a proper model to model the whole system
% - alternatively, might want to use galactic bfe as a basis in which to evolve individual subhalos -- could work for low mass ones
...
% - remains to see how to constrain, as many orders required
