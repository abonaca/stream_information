...
At this precision, simulated dark matter halos deviate from simple analytic parametrizations, so we add an expansion of basis functions \changes{as a first step in giving the gravitational potential sufficient freedom}.
...
They simulated tidal disruption of a Sagittarius-like satellite in different models of the Milky Way, and showed that with precise observations of the stream's positions and kinematics, the mass and shape of the halo can be constrained with \changes{a precision of approximately one percent}.
...
\changes{Streams originating from more massive dwarf galaxies are not directly addressed in this study, but results derived in \S\S\,\ref{sec:results},~\ref{sec:applications} should hold for streams in general as \citet{fardal2015} showed how a similar framework can be used to model streams from both globular clusters and dwarf galaxies.}
...
\changes{A number of methods for creating models of streams given a progenitor's orbit in an analytic potential have been well established in the literature \citep[e.g.,][]{varghese2011, kupper2012, bonaca2014, gibbons2014, bovy2014, fardal2015}.
In this work, we use a modified version of our \FF\ framework from \citet{bonaca2014} to create mock stream analogs of ATLAS, GD-1, Hermus, Kwando, Orinoco, PS1A, PS1C, PS1D, PS1E, Sangarius and Triangulum streams.
The choice of these particular streams and the generation of model streams are explained in Appendix~\ref{sec:streams}.
The most important aspect of these models for their application in the Fisher analysis is that they are generative, that is, a stream model is uniquely defined given a set of input parameters.
...
\changes{A stream model is a distribution of tracer particles in a six-dimensional phase space (a 2D sky projection of the ATLAS-like stream is shown with gray points in Figure~\ref{fig:derivative_steps}).
...
\end{enumerate}
\changes{In each of these scenarios, we adopted observational uncertainties for every type of available data (as listed in Table~\ref{t:datasets}).
...
\changes{For the halo scale velocity, DESI-like observations produce \CRLB\ only 7\,\% larger on average than the fiducial 4D observations, and the difference is only 1\,\% for $x/y$ axis ratio of the halo.}
\changes{Similarly,} Gaia-like observations inherit uncertainties of the fiducial 6D observing mode, but assume distances are known an order of magnitude more precisely, while the proper motions are a factor of 2 worse than in the fiducial \changes{observing mode}.
...
The dispersion between these two observing \changes{modes} is 20\,\% -- larger than the median difference, and indeed, Figure~\ref{fig:obsmodes} shows that some streams are more informative with Gaia-like \changes{observing mode}, while others provide better constraints on the halo with fiducial 6D \changes{observing mode}.
...
\changes{The resulting set of 11 mock streams is characterized in Table~\ref{t:mocks_input}, which lists the adopted, high-likelihood values for progenitor parameters.
