...
& r^2 = x^2 + \left(\frac{y}{q_y^2}\right)^2 + \left(\frac{z}{q_z}\right)^2
\end{aligned}
\label{eq:nfw}
\end{equation}
...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{../plots/observable_steps_-1_p0_Ns12.png}
\caption{
}
...
We start by creating two stream models, where we change parameter $x_i$, $x_i = x_{i,0} \pm \Delta x_i$, and keep the rest of the parameters at their fiducial values.
The first row of Figure~\ref{fig:derivative_steps} shows models of the GD-1 stream with different offsets $\Delta V_{h,i}$ from the fiducial scale velocity (color-coded such that the positive offsets are in shades of red, and negative in shades of blue).
...
We describe how we fit these great circles, and provide rotation matrices from the equatorial system to this stream coordinates in Appendix~\ref{}.
- differences shown in the second row
...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{../plots/step_convergence_-1_Ns20_logTrue_l2.png}
\caption{
}
...
For simplicity, we assume that for each stream, there are 50 observations uniformly distributed along the length of the stream.
