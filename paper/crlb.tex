...
\begin{abstract}\noindent % trust me
Cold stellar streams---produced by tidal disruptions of \changes{globular} clusters---are long-lived, coherent dynamical features in the halo of the Milky Way.
...
They \changes{hold the promise of delivering} precise information about the gravitational potential, including constraints on the shape of the dark-matter halo.
...
At this precision, simulated dark matter halos deviate from simple analytic parametrizations, so we add an expansion of basis functions \changes{as a first step in giving the gravitational potential more freedom}.
...
We show that velocities of stream stars are essential for producing competitive constraints on the distribution of dark matter.
% , which bodes well for stream studies in the age of \textsl{Gaia}.
\end{abstract}

\keywords{Galaxy: halo --- dark matter --- Galaxy: kinematics and dynamics --- methods: statistical}

\section{Introduction} \label{sec:intro}
Immediately following the discovery of the first accreted structures in the Milky Way's halo \citep{ibata1994, ti1998}, \citet{johnston1999} recognized that these tidal tails can inform \changes{us} about the properties of the halo itself, concluding \emph{``you can judge a galaxy by its tail''}.
They simulated tidal disruption of a Sagittarius-like satellite in different models of the Milky Way, and showed that with precise observations of the stream members' positions and kinematics, the mass and shape of the halo can be constrained with \changes{a precision of approximately one percent}.
...
Gathering the required observational data was the main cause of the delay\changes{;} streams are diffuse and faint, so it is challenging to isolate stream members from the overwhelming Milky Way field.

To this day, a total of four streams have been used to measure the Milky Way's gravitational potential: tidal tails of the Sagittarius dwarf galaxy \citep{lm10,gibbons2014,dl2017}, the Orphan stream \citep{newberg2010}, the GD-1 stream \citep{koposov2010, bowden2015} and the tails of Palomar~5 globular cluster \citep{kupper2015}.
...
To quantitatively address this question, we here build a framework for measuring the information content in \changes{cold} stellar streams regarding the gravitational potential.
...
We use the formalism of Cram\'er--Rao lower bounds and describe how we applied it to stellar streams in Section~\ref{sec:method}.
...
Finally, relaxing our model for the gravitational potential \changes{to begin capturing the likely complex shape of the dark matter halo}, we show that streams best constrain the radial acceleration at their present location (\S\S\,\ref{sec:interpretation},~\ref{sec:bfe}).
...
We define the information content in stellar streams as the best-case uncertainties on the model parameters achievable using the observational data at hand.
...
For some data set $\vec{y}$ (e.g., a vector of position and velocity measurements of stars along a stream), the associated noise covariance matrix is $C_y$.
...
Although the \CRLB\ is a constraint on frequentist estimators, it also has an interpretation in Bayesian inference.
...
\changes{Streams originating from more massive dwarf galaxies are not directly addressed in this study, but results derived in \S\S\,\ref{sec:results},~\ref{sec:applications} should hold for streams in general. \citet{fardal2015} showed how a common framework can be used to model streams from both globular clusters and dwarf galaxies.}
...
\changes{A number of methods for creating models of streams given a progenitor's orbit in an analytic potential have been established in the literature \citep[e.g.,][]{varghese2011, kupper2012, bonaca2014, gibbons2014, bovy2014, fardal2015}.
In this work, we use a modified version of the \FF\ framework from \citet{bonaca2014} to create mock stream analogs of ATLAS, GD-1, Hermus, Kwando, Orinoco, PS1A, PS1C, PS1D, PS1E, Sangarius and Triangulum streams.
The choice of these particular streams and the generation of model streams are explained in Appendix~\ref{sec:streams}.
The most important aspect of these models for their application in the Fisher analysis is that they are generative, that is, stream observables are uniquely defined given a set of input parameters.
% This property allows for the calculation of derivatives.
None of the results presented in the rest of the paper would change substantially if a different generation method, or a different set of streams, were chosen.
}
...
\changes{In our context, a stream model is a distribution of tracer particles in a six-dimensional phase space (a 2D sky projection of the ATLAS-like stream is shown with gray points in Figure~\ref{fig:derivative_steps}).
The stream model is completely determined by the underlying gravitational potential, current 6D position of the progenitor, its initial mass and the stream age.
With a given set of other parameters, the progenitor mass and the stream age set the stream's length, and are highly degenerate.
As the stream's length is observationally uncertain, we opted to calculate information in streams without using their length directly (see next section), and to fix the progenitor mass and age to the best-fitting values for each mock stream.}
...
\changes{Thus, we only use the present-day position of the progenitor as model parameters,} defined in the space of observables with the progenitor's two on-sky position angles ($RA_p$, $Dec_p$), distance $d_p$, radial velocity $V_{r,p}$ and two proper motion components ($\mu_{\alpha,p}$, $\mu_{\delta,p}$). 
...
\caption{We quantify the response of streams \changes{to} changes in model parameters by calculating numerical derivatives, and in this figure we visualize the $d\eta / d V_h$ derivative (change in the sky position with the change in scale velocity of the underlying dark matter halo).
...
Consequently, \changes{we expect that} not considering variations along the stream results in only a negligible loss of information.
...
However, to a first order, streams are one dimensional structures, and lines in the top panel of Figure~\ref{fig:derivative_steps} show that a B-spline fits well the distribution of stream positions.
...
\changes{Derivatives of functions evaluated by any computer program, including orbit integrators used to create stream models in this work, can in principle be calculated using a set of techniques for \emph{automatic differentiation} \citep{griewank2008}.
...
The amount of information we can learn about the gravitational potential from a stream data set depends both on the intrinsic sensitivity of a stream to different aspects of the gravitational potential, and also on the properties of the data set itself.
...
In this Section, we set up different stream data sets, defined by the type, number and precision of observations at hand.

For simplicity, we assume that observations across the different data sets are uniformly distributed every 0.5\,deg along \changes{the full length of each mock stream}, with a minimum of 15 measurement points per stream.
...
All of these assumptions make for a highly idealized scenario: there are density inhomogeneities along the streams, so the likely members are not in general equidistant; different types of data are typically obtained for different stars; and observational uncertainties are a function of stellar brightness, color, and often distance.
...
\hline
\end{tabular}
\caption{Observational precisions adopted}
\label{t:datasets}
\end{center}
\end{table}
...
\item current-quality data on positions of stream stars with radial velocities from DESI and DESI-like surveys, and
\item data on stream members from the final data release of the Gaia mission.
\end{enumerate}
\changes{In each of these scenarios, we adopted observational uncertainties for every type of available data (listed in Table~\ref{t:datasets}).
...
% As we are interested , we adopt a single value of $2\,\rm kpc$ as our fiducial uncertainty in distance for simplicity.
This value is conservative for nearby streams, but likely overly optimistic for the more distant ones.
\changes{In a more realistic scenario, the distance uncertainty would scale with the distance to the stream, but in this general exploration of information in stellar streams we assume homoscedastic uncertainties.}
...
The uncertainty in stream's celestial position will still be limited by its intrinsic width, but the distance estimates should be improved by an order of magnitude with respect to the fiducial case.
...
Lastly, proper motion precision is $\approx0.2$\,mas for faint, blue stars, such as those at the main sequence turn-off of stellar streams.
...
\section{Results}
\label{sec:results}
Following the steps outlined in Section~\ref{sec:method}, we inferred the information content in 11 Milky Way-like streams.
...
\caption{A visualization of the Cram\'er--Rao bounds (ellipses) on all 15 of the model parameters for an ATLAS-like stream.
...
Given that the pericenter of this stream is at 23\,kpc, which is approximately 30 and 7 times beyond the bulge and disk scale lengths, respectively, it is unsurprising that its debris provides little information on these components.
...
This result is somewhat surprising, because in the absence of velocities, the masses and timescales in the system can be arbitrarily rescaled to reproduce the same positions, so we expect that kinematic data is a prerequisite for any dynamical inferences.
...
We quantify these covariances with the Pearson correlation coefficient, which for a pair of parameters $(i,j)$ is simply $\rho = C_{x,ij} / \sqrt{C_{x,ii}\,C_{x,jj}}$, where $C_{x,kl}$ is the $k,l$ element of the Fisher matrix $C_x$.
...
One would expect similar behavior for the 3D spatial position of the progenitor\changes{;} however, distances in our fiducial case are too uncertain to exhibit correlations with on-sky positions.
...
\label{sec:res_comp}
In the context of our model, each stream provides information on the position of its individual progenitor, as well as on the distribution of matter in the Galaxy that they all have in common.
...
In the previous Section, we explored correlations in Cram\' er--Rao bounds between all parameter pairs (Figure~\ref{fig:crb_correlations}), so we start this section by comparing different streams' constraints on a single pair of parameters.
...
An in-depth analysis of these bounds can inform an optimal observing strategy, and we explore implications that currently ongoing spectroscopic and astrometric surveys will have for tidal streams in Section~\ref{sec:forecast}.
...
In the remainder of this Section, we examine why different streams are differently sensitive to various model parameters, and study how the \CRLB\ for individual parameters depend on properties of the stream and the orbit of its progenitor.
...
In general, the KLD compares how similar two distributions are, so in our case, a small divergence means that the bound contains little information from streams.
...
None of the streams improved upon the prior information for the length scale of the bulge and disk, only several have improved on the bulge mass measurement (the inner halo streams GD-1, Hermus, Orinoco, and PS1A), while all the streams have constrained the disk mass.
...
\changes{Data points are color-coded by stream length, with longer streams being represented with darker colors.}
The correlation in every panel is indicated in the top right corner with a correlation coefficient and with the panel brightness.
...
The Pearson correlation coefficient ($\rho$) between the model parameter and the stream property is given in the top right corner of every panel.
To emphasize stronger correlations, the text size of the quoted correlation coefficient and the brightness of the panel's background increase with the absolute size of the coefficient.
...
Given a fixed observational uncertainty, this means that longer streams should in general be more sensitive to changes in the gravitational potential -- a hypothesis quantitatively confirmed with the \CRLB\ of all model parameters improving with stream length.
...
The Fisher information matrix, given by Equation~\ref{eq:crlb}, is proportional to the logarithm of the probability of observing data $\vec{y_i}$ of stream $i$, given true parameters of the Galactic model $\vec{x}$, that is $C_{x,i}^{-1}\propto \ln(P(\vec{y_i}|\vec{x}))$.
...
\end{equation}
\changes{where $(d\vec{y}/d\vec{x})_i$ are the derivatives of observables $\vec{y}$ with respect to parameters $\vec{x}$ for stream $i$, $C_{y,i}$ is the covariance matrix for the observations of stream $i$, and $V_x$ is the covariance matrix with prior constraints on parameters $\vec{x}$.}
...
There are $\binom{11}{N}$ unique combinations with a total of $N$ streams in a combination, so we highlight the median constraint at a given $N$ with larger white points.
...
The three main scenarios for data availability we considered are: fiducial \changes{in 3D, 4D and 6D}, DESI-like and Gaia-like, which differ both in the dimensionality of the observational data as well as its precision (for more details, see Section~\S\,\ref{sec:datasets} and Table~\ref{t:datasets}).
...
The \CRLB\ under fiducial observational uncertainties are shown with solid lines in Figure~\ref{fig:obsmodes}, with the thinnest and lightest lines mapping constraints from 3D data, medium from 4D, and thickest and darkest from 6D data.
...
DESI-like observations are \changes{directly comparable} to the fiducial 4D observing mode.
\changes{This observing mode assumes that positions and radial velocities of stream stars are available, and that the uncertainties in the positions are the same as in the fiducial observing mode, but adopts a factor of 2 larger uncertainties in radial velocities.}
This uncertainty, however, has little impact on the recovered \CRLB.
\changes{For the halo scale velocity, DESI-like observations produce a \CRLB\ only 7\,\% larger on average than the fiducial 4D observations, and the difference is only 1\,\% for the $x/y$ axis ratio of the halo.}
\changes{Similarly,} Gaia-like observations inherit uncertainties of the fiducial 6D observing mode, but assume that distances are known an order of magnitude more precisely, while the proper motions are a factor of 2 worse than in the fiducial \changes{observing mode}.
Again, these differences in data precision have a modest impact \changes{on} the resulting \CRLB: Gaia-like \changes{observations are} on average slightly less constraining than the fiducial \changes{6D observations}, but only marginally so.
The largest difference of 20\,\% between the Gaia-like and fiducial 6D \changes{observations} is in halo $z$-axis flattening, whereas the smallest is 3\,\% for $x$-axis flattening.
The dispersion between these two observing \changes{modes} is 20\,\% -- larger than the median difference, and indeed, Figure~\ref{fig:obsmodes} shows that some streams are more informative with a Gaia-like \changes{observing mode}, while others provide better constraints on the halo with the fiducial 6D \changes{observing mode}.
...
In general, Cram\'er--Rao bounds on a set of variables $\vec{q}$ can be propagated to a new set of variables $\vec{p}(\vec{q})$ using \citep[e.g.,][]{albrecht2009}:
\begin{equation}
...
The overall acceleration is also decreasing, so we show the \CRLB\ on the radial acceleration relative to the acceleration in the top right.
The relative uncertainty on the acceleration is large at small radii\changes{;} it then decreases to a local minimum and asymptotically increases at large radii.
The exact \changes{radial dependence of the acceleration precision} varies between different streams, but the presence of a local minimum is universal.
\changes{This means that streams do not measure accelerations equally well everywhere in the Galaxy, but instead, for every stream there is a range in Galactocentric distance where the gravitational potential is best constrained.}
% indicating that the information streams provide about the gravitational potential is radially confined.
...
Since the galaxy model is not spherically symmetric, we calculated constraints on the radial acceleration along 50 random lines of sight and marked the median with circles, while the error bars indicate 16th and 84th percentile in all three panels of the second row.
The bottom-left panel presents the best relative \CRLB\ for a given stream as a function of the stream length.
...
Different functional forms relate the best constrained radius to the current radius and the apocenter (non-linear and approximately linear, respectively), so it is unclear which, or if either, of these relations is intrinsic.
...
As a final application of our framework to measure information content in streams, in the next Section we relax our assumptions about the simplicity of the gravitational potential and distinguish between these radically different interpretations using a more flexible potential model.
...
In principle, a multipole decomposition of a high order ($l\approx 10$) can reproduce a static snapshot of dark-matter halos expected in the $\Lambda$CDM paradigm even on small scales \citep{lowing2011}.
...
The finding that streams constrain the mass enclosed within their current positions, with a precision determined by their length, has far-reaching implications for observational studies as both the position and length are directly observable.
Equipped with this knowledge, we can tailor observational experiments and prioritize telescope resources to study the most informative streams.
...
In this, the first analysis of its kind, we made a number of simplifying assumptions, which we critically revisit in \S\,\ref{sec:dis_caveats}.
...
Many of these can be explored using the Fisher matrices from this work, which are archived in Zenodo (\dataset[doi:10.5281/zenodo.1413746]{https://doi.org/10.5281/zenodo.1413746})\footnote{See also \url{https://github.com/abonaca/stream_information}.}
...
In our fiducial, parametric model of the Galaxy, the radial acceleration is best measured at distances similar to those at which the stream resides currently, but in more flexible potentials the best constraints hone in on the current location of the stream.
...
With a moderately flexible and static model, we were able to demonstrate this localization of stream information in the radial direction, whereas future studies with fully flexible potentials will test whether stream constraints are local in all three dimensions.
...
The Fisher analysis here was performed in the context of a model, because it ultimately relies on calculating derivatives of the data with respect to the parameters of the model, and these derivatives are evaluated at the fiducial values of the model parameters.
...
Another caveat of this work is that its stream data model is only a naive reflection of a more complicated reality. 
...
Principal derivatives of stream observables with respect to model parameters will not only make for more safely calculated \CRLB, but could also aid in fitting streams, as higher-order parameter-estimation methods, such as Hamiltonian Monte Carlo, rely on evaluating both the likelihood and the likelihood derivative.
...
We relegate the development of such a model and its implementation for mapping the dark matter in the Galaxy to a future study.
...
Such forecasting, while standard in other areas of astronomy \citep[e.g.,][]{bond1997,tegmark2000}, is novel in near-field cosmology.
...
In this new regime, it will be essential to identify the most efficient targets among the streams, and also within the stream itself, to optimally use the shared resources -- a task ideally suited to the framework developed in this work.
...
All code used in this work and all results are available on GitHub \footnote{\url{https://github.com/abonaca/stream_information}} and archived in Zenodo (\dataset[doi:10.5281/zenodo.1413746]{https://doi.org/10.5281/zenodo.1413746}).
}

\software{
    \package{Astropy} \citep{astropy},
    \package{gala} \citep{gala},
    \package{h5py} (\url{https://www.h5py.org/}),
...
We limited our study to eleven streams in the PanSTARRS footprint: ATLAS \citep{koposov2014}, GD-1 \citep{grillmair2006}, Hermus \citep{grillmair2014}, Kwando, Orinoco \citep{grillmair2017b}, PS1A, PS1C, PS1D, PS1E \citep{bernard2016}, Sangarius \citep{grillmair2017a} and Triangulum \citep{bonaca2012}.
...
\begin{figure}
\begin{center}
\includegraphics[width=\textwidth]{mocks_orbits.pdf}
\caption{Positions of mock streams (black points) and their progenitors' orbits (gray lines) in Galactocentric cylindrical coordinates.
Mock streams in the analyzed sample span a range in average Galactocentric distance, eccentricity and current orbital phase.
}
\label{fig:orbitgallery}
\end{center}
\end{figure}
...
The color-magnitude probability is obtained from a matched filter based on the M13 globular cluster, and placed at the distance to the stream \changes{reported in discovery papers referenced above, and also noted in Table~\ref{t:mocks_input}}.

\item Select \changes{a hundred} most likely members.
\changes{In the case of GD-1, we selected 24 kinematic members from \citet{koposov2010}, and 76 members following the above procedure.
We assign a likely distance to every member star by drawing from a normal distribution centered on the reported distance to the stream, and assuming a 20\,\% width to accommodate for any unresolved distance gradients.
}
...
We created \changes{streakline} models of mock streams in a fiducial gravitational potential defined by the first 9 parameters listed in Table~\ref{t:model} and kept these parameters fixed for all of the mock streams.
The parameters we varied were: 6D position of the progenitor today, initial mass of the progenitor, and the age of the stream.
\changes{For simplicity, we assume the progenitor loses mass at a constant rate and disappears at the present.
These choices predominantly affect the density along the stream, and not the location of its track that was used in this work.
Following \citet{bonaca2014}, we searched for likely progenitor configurations by comparing distributions of observed and model stream stars.
}
...
\changes{The resulting set of 11 mock streams is given in Table~\ref{t:mocks_input}, which lists the adopted, high-likelihood values for progenitor parameters.
In each panel, Figure~\ref{fig:gallery} compares the on-sky locations of an observed stream (black points) and an analog mock stream (gray points).
Mock streams span a range of orbital periods and eccentricities (Table~\ref{t:mocks_orbit}).
Each panel of Figure~\ref{fig:orbitgallery} shows the progenitor track in the Galactocentric $R-z$ plane (gray) and the present-day locations of stars in the mock stream (black).
The diversity in observed properties of mock streams is matched by the diversity in orbits of disrupting progenitors.
}

In most cases, mock streams track well the positions of their observed counterparts \changes{(Figure~\ref{fig:gallery})}.
...
This work focused on assessing the precision, and not accuracy, of stream constraints, so it is acceptable that our fiducial potential is only approximately representative of the Milky Way, \changes{and that our mock streams are only approximately representative of streams observed in the Milky Way}.
...
\hline
\hline
Name & $RA_p$ & $Dec_p$ & $d_p$ & $V_{r,p}$ & $\mu_\alpha$ & $\mu_\delta$ & Initial mass & Age \\
& (deg) & (deg) & (kpc) & (km\,s$^{-1}$) & (mas\,yr$^{-1}$) & (mas\,yr$^{-1}$) & ($\rm M_\odot$) & (Gyr) \\
\hline
\input{mock_inputs}
\hline
\hline
\end{tabular}
\caption{Input parameters for generating Milky Way-like mock stellar streams}
\label{t:mocks_input}
\end{center}
\end{table}
...
\hline
\hline
Name & Period & $R_{peri}$ & $R_{apo}$ & $R_{cur}$& Eccentricity \\
& (Gyr) & (kpc) & (kpc) & (kpc) & \\
\hline
\input{mock_orbit}
\hline
\hline
\end{tabular}
\caption{Orbital parameters of the Milky Way-like mock stellar streams}
\label{t:mocks_orbit}
\end{center}
\end{table}
